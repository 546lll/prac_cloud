...
<script>
import { getDynamicCom } from '@/api/dynamic.js'

import sendComment from '@/components/context/sendComment/SendComment.vue' // 发送评论
    ...
    show () {
      if (this.show) {
        ...
        getDynamicCom(this.comid).then((res) => {
          this.comTitle = '评论(' + res.data.total + ')'
          this.comHot = res.data.hotComments.map(item => {
            return {
          ...
          this.comList = res.data.comments.map(item => {
            return {
              content: item.content, // 评论内容
              likedCount: item.likedCount, // 点赞数
              commentId: item.commentId, // 评论id
